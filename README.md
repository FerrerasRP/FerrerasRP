<p align="right">
  🌐 <b>Idiomas:</b> <a href="README.md">Português - Brasil</a> | <a href="docs/README.en.md">English</a> | <a href="docs/README.es.md">Español</a>
</p>

<h1 align="center">Olá, eu sou Ricardo 🇧🇷👨‍💻♑✝️📝📚💻</h1>

<p align="center">
  <i>Sou um entusiasta de tecnologia e dados, sempre em busca de novas soluções e inovações.</i>
</p>

---

<table width="100%">
  <tr>
    <td width="50%" valign="top">
      <h3>🤝 Habilidades Interpessoais</h3>
      <ol>
        <li><b>Análise Crítica:</b> Tenho uma capacidade analítica afiada para resolver problemas complexos.</li>
        <li><b>Comunicação Empática:</b> Me comunico de forma clara e compreensiva.</li>
        <li><b>Colaboração Eficaz:</b> Acredito na força do trabalho conjunto, onde a colaboração e a troca de ideias levam a melhores resultados para todos.</li>
      </ol>
    </td>
    <td width="50%" valign="top">
      <h3>⚙️ Habilidades Técnicas</h3>
      <ol>
        <li><b>Análise de Dados:</b> Estudo e extraio valor dos dados para insights valiosos.</li>
        <li><b>Automatização:</b> Desenvolvo rotinas e processos automatizados.</li>
        <li><b>Inovação nos Processos:</b> Utilizo tecnologia de ponta para transformar e otimizar processos empresariais, garantindo eficiência e resultados superiores.</li>
      </ol>
    </td>
  </tr>
</table>

---

<p align="center">
  <a href="https://github.com/FerrerasRP/"><img src="https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"></a>
  <a href="https://www.kaggle.com/rferreras"><img src="https://img.shields.io/badge/Kaggle-%23035a7d?style=for-the-badge&logo=kaggle&logoColor=white" alt="Kaggle"></a>
  <a href="http://www.linkedin.com/in/ricardoferreras"><img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
  <a href="http://x.com/ricardoferreras"><img src="https://img.shields.io/badge/X-000000?style=for-the-badge&logo=x&logoColor=white" alt="X"></a>
  <a href="https://pypi.org/user/rferreras/"><img src="https://img.shields.io/badge/pypi-3775A9?style=for-the-badge&logo=pypi&logoColor=white" alt="Pypi">

</p>

<p align="center">
  <img src="./metrics.plugin.FerrerasRP.svg"/>
</p>
<p align="center">
  <img src="https://komarev.com/ghpvc/?username=FerrerasRP&style=flat-square" alt="counter" />
</p>
